...
//
// Workflow Task Name: Ansible Tower Register Host
//
// Version: 1.0
//
// Description:
//	This custom task leverages the Ansible Tower API to register a new host.
//
...
//	Hostname: Short hostname of newly created VM
//          NewVMIP: IP address of new VM
//          NewVMDescription: Description of new host
//          AnsibleTowerInventory: Specify inventory to add new host
//  
// Outputs:
...
function WebServiceAccess() {
	...
	var strURL = transport+"://"+towerFQDN+"/api/v1/hosts/";
	var authorizationHeader = TowerAuthString;

	var hostname = String(input.Hostname);   //should be just the short hostname
    	var newVM_ip = String(input.NewVMIP);
    	var newVM_description = String(input.NewVMDescription)
	var host_inventory = input.AnsibleTowerInventory;
   
    	var enabled_value = true;
    	var instance_id = "";
    	var variables = "---";
    	var last_job = null;
    	var last_job_host_summary = null;
    
	// Create JSON Payload
	var map = new HashMap();
	map.put("name", newVM_ip);
	map.put("description", newVM_description);
	map.put("inventory", host_inventory);
	map.put("enabled", enabled_value);
	map.put("instance_id", instance_id);
	map.put("variables", variables);
	map.put("last_job", last_job);
	map.put("last_job_host_summary", last_job_host_summary)
